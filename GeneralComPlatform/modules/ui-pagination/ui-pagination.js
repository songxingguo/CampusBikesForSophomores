...
.directive("uiPagination", function(){
    return {
        ...
        template: '<div class="clearfix pagination-wrap">' +
            '<div class="pagination info">{{"共"}} <span style="color:#1d6093">{{totalItems}}</span> {{"条"}} ' +
            '[{{"每页"}} <span style="color:#afb7be">{{itemsPerPage}}</span> {{"条"}}]</div>' +
            '<div class="pagination jump" ng-if="numPages> 5">' +
            '{{"共"}} {{numPages}} {{"页" }} {{"去"}}{{"第"}}<form name="jumpForm" style="display: inline"><input type="text" name="jumpInput" ng-model="jumpInput" required ng-pattern="/^[0-9]*$/"></form>{{"页"}}' +

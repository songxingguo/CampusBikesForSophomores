...
(function($, undefined){
	...
	var winheight = function() {
		return window.innerHeight ? window.innerHeight : $(window).height() || (window.screen.height-120);
	};
	...
	MSGBOX.prototype = {
			...
			switch (this.options.type) {
				...
				case 'prompt':
					...
					if (this.options.buttons!==null && this.options.buttons.length == 0) 
						this.options.buttons = ['Cancel', 'OK'];
		...
		animate: function (sc, callback, speed) {
			...
			var left = (($(window).width() || window.screen.width) - sc.width) / 2;
